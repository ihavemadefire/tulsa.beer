        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function(i, item){
                    $('#beersloader').append('<div class=" mt-3 mr-5"><div class="mt-3 card" style="width: 18rem;"><div class="card-body"><h5 class="card-title">'+ item.name +'</h5><h6>' + item.brewer + '</h6><p>' + item.style + '</p><p class="card-text">IBU: ' + item.ibu + '</p><p class="card-text">ABV: ' + item.abv + '</p><a href="#" class="btn btn-warning">More</a></div></div></div>');
                });
            }
        });
    };
